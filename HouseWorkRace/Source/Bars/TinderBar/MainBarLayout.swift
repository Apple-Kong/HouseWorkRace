...
class MainBarLayout: TMBarLayout {
    ...
    private var containers = [MainBarButtonContainer]()
    ...
    override func insert(buttons: [TMBarButton], at index: Int) {
        buttons.forEach { (button) in
            let container = MainBarButtonContainer(for: button)
    ...
    override func remove(buttons: [TMBarButton]) {
        let containers = stackView.arrangedSubviews.compactMap({ $0 as? MainBarButtonContainer })
